...
const ACCOUNT_LIMIT_DEFAULTS = {
  minimum_deposit_amount: 100,
  minimum_bet_amount: 1,
  minimum_withdrawal_amount: 500
};

async function getAccountLimits(db = pool) {
  const result = await db.query(
    "SELECT setting_key, setting_value FROM settings WHERE setting_key = ANY($1::text[])",
    [Object.keys(ACCOUNT_LIMIT_DEFAULTS)]
  );
  return Object.fromEntries(
    Object.entries(ACCOUNT_LIMIT_DEFAULTS).map(([key, fallback]) => {
      const value = Number(result.rows.find(row => row.setting_key === key)?.setting_value);
...
app.get('/api/account-limits', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      minimumDeposit: limits.minimum_deposit_amount,
      minimumBet: limits.minimum_bet_amount,
      minimumWithdrawal: limits.minimum_withdrawal_amount
    });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Could not load account limits' });
  }
});
...
app.post('/bet', async (req, res) => {
  ...
  try {
    ...
    if (!Number.isFinite(betAmount) || betAmount < limits.minimum_bet_amount) {
      return res.status(400).json({
        error: `Minimum bet is KSH ${limits.minimum_bet_amount.toFixed(2)}`
      });
    }
...
app.post('/withdraw', async (req, res) => {
  ...
  try {
    const limits = await getAccountLimits();
    if (!Number.isFinite(withdrawAmount) || withdrawAmount < limits.minimum_withdrawal_amount) {
      return res.status(400).json({ error: `Minimum withdrawal is KSH ${limits.minimum_withdrawal_amount.toFixed(2)}` });
    }
    client = await pool.connect();
    await client.query('BEGIN');
...
app.get('/admin/account-limits', async (req, res) => {
  const pwd = req.headers['authorization'];
  if (pwd !== '3462Abel@#') return res.status(401).json({ error: 'Unauthorized' });
  try {
    const limits = await getAccountLimits();
    ...
    res.status(500).json({ error: 'Failed to load account limits' });
  }
});

app.post('/admin/account-limits', async (req, res) => {
  const pwd = req.headers['authorization'];
  if (pwd !== '3462Abel@#') return res.status(401).json({ error: 'Unauthorized' });

  const values = {
    minimum_deposit_amount: Number(req.body.minimumDeposit),
    minimum_bet_amount: Number(req.body.minimumBet),
    minimum_withdrawal_amount: Number(req.body.minimumWithdrawal)
  };
  if (Object.values(values).some(value => !Number.isFinite(value) || value <= 0)) {
    return res.status(400).json({ error: 'All minimum amounts must be greater than zero' });
  }
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to save account limits' });
  }
});
...
async function getPredictorPlans() {
  const result = await pool.query(
    "SELECT plan_key, name, price_ksh, duration_days, features, active FROM predictor_plans ORDER BY price_ksh ASC"
  );
...
app.get('/api/predictor/signal', async (req, res) => {
  const formattedPhone = formatPhone(req.query.phone || '');
  if (!formattedPhone) {
    return res.status(401).json({ success: false, subscribed: false, error: 'Login required' });
  }
  ...
  try {
    ...
    if (!subscription || settings.predictor_enabled === 'false') {
      return res.json({ success: true, subscribed: false, signal: null });
    }

    const recent = oddsHistory
      .slice(0, 5)
    ...
    const recentAverage = recent.length
      ...
      : 2;
    const offset = Math.min(5, Math.max(0, Number(settings.predictor_target_offset) || 0));
    const target = settings.predictor_mode === 'manual'
      ? Math.min(50, Math.max(1.01, Number(settings.predictor_manual_target) || 2))
      : Math.min(50, Math.max(1.1, recentAverage - offset));
    const confidence = settings.predictor_mode === 'manual'
      ? Math.min(99, Math.max(1, Number(settings.predictor_manual_confidence) || 50))
      : Math.min(99, Math.max(1, Math.round(55 + Math.min(recent.length, 5) * 5)));
    const signal = settings.predictor_mode === 'manual'
      ? (settings.predictor_manual_signal || 'WATCH')
      : (target >= 2 ? 'ENTER' : 'WATCH');
    const roundId = gameStatus === 'RUNNING' && gameStartedAt
      ? `round-${gameStartedAt}`
      : null;
    ...
    res.json({
      ...
      signal: {
        id: roundId,
        roundActive: Boolean(roundId),
        label: signal,
        target: Number(target.toFixed(2)),
        confidence,
        note: settings.predictor_note || '',
        generatedAt: Date.now()
      }
    });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Could not load the live prediction' });
  }
});
...
app.post('/admin/predictor/config', async (req, res) => {
  ...
  try {
    ...
    for (const plan of Array.isArray(plans) ? plans : []) {
      ...
      await client.query(`
        INSERT INTO predictor_plans (plan_key, name, price_ksh, duration_days, features, active, updated_at)
        ...
        ON CONFLICT (plan_key) DO UPDATE SET
          ...
          updated_at = NOW()
      `, [planKey, name, price, duration, JSON.stringify(features), active]);
    }
    await client.query('COMMIT');
    res.json({ success: true, settings: await getPredictorSettings(), plans: await getPredictorPlans(), stats: await getPredictorStats() });
  } catch (e) {
...
app.post("/pay", async (req, res) => {
  try {
    ...
    const limits = await getAccountLimits();
    if (!Number.isFinite(numericAmount) || numericAmount < limits.minimum_deposit_amount) {
      return res.status(400).json({
        success: false,
        error: `Minimum deposit is KSH ${limits.minimum_deposit_amount.toFixed(2)}`
      });
    }
