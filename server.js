  ...
  try {
    ...
    if (!resp.data?.success) {
      return {
        success: false,
        status: 400,
        error: resp.data?.error || "Failed to initiate STK payment"
      };
    }
    ...
    if (err.response?.status === 401) {
      return {
        success: false,
        status: 502,
        error: "SwiftWallet rejected the API key. Check SWIFTWALLET_KEY in Render."
      };
    }
    return {
      success: false,
      status: 502,
      error: err.message || "Could not send STK push"
    };
  }
}
...
app.post('/signup', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [username, formattedPhone, pin, signupBonus, actualReferralCode]
    );
...
app.post('/withdraw', async (req, res) => {
  ...
  try {
    ...
    if (!stk.success) {
      ...
      return res.status(stk.status || 502).json({ error: stk.error });
    }
...
app.post('/admin/threshold-settings', async (req, res) => {
  ...
  if (req.body.deduct_bonus === true) {
    return res.status(400).json({
      error: 'Bonus-balance deduction is unavailable because this database has no separate bonus balance field.'
    });
  }
...
app.post("/withdrawal-retry-stk", async (req, res) => {
  ...
  try {
    ...
    if (!stk.success) {
      ...
      return res.status(stk.status || 502).json({ success: false, error: stk.error });
    }
